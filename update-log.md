...
- 新增
  ...
  - 事件管理
    - 埋点事件打标：支持给埋点事件增加标签，以提高对事件的管理效率。
  - 高级配置
    - 全局参数：通过修改全局参数可动态修改产品功能中的约束条件，比如创建虚拟事件时可选用的事件数量限制。
  - 项目管理
    - 导出指定资源和元数据：支持导出指定的看板以及其依赖的单图、计算指标、分群，以及它们依赖的事件/用户/维度表等，然后在新的环境中导入。
  - 事件流分析
    - 事件流保存到看板：可以把事件流制作的图表添加到看板上，统一在看板进行管理。
  - 看板
    - 看板支持公共分组：可以借助管理员权限的账户，编辑、发布公共分组的看板列表（列表为空则不显示公共分组）。
  - KPI 分析
    - KPI 分析支持指标预警：在事件分析预警的基础上，可以针对 KPI 分析创建预警，并进行统一管理。
  - 漏斗分析
    - 漏斗分析支持指标预警：在事件分析预警的基础上，可以针对漏斗分析创建预警，并进行统一管理。

- 优化

  - 用户属性
    - 用户属性支持小数：用户属性创建的类型选择中，支持选用小数。

## v2.10: 2022 年 08 月 10 日发布
