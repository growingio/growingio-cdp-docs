...
- 新增
  ...
  - 统一门户（Portal）
    - 全局水印：企业级全局水印，开启后页面显示水印：账号+姓名，降低数据泄露风险。
    - 工作台：个人工作台，管理常用功能，提升工作效率。
  - 增长分析
    - 间隔分析：支持选择初始事件和后续事件，通过箱型图展示两个事件的转化时长分布。
    - 会话分析：增加属性（维度）拆解和过滤的能力
  - 获客分析
    - 深度链接：支持 APP 端创建深度监测链接。
    - 产品配置：支持移动端深度链接配置。
...
- 优化
  ...
  - 智能运营
    - 产品信息架构改版，统一信息架构。
  - 获客分析
    - 监测链接：支持 APP、网页及小程序端广告投放链接监测
    - 产品配置：支持配置推广渠道、广告活动及归因配置
  - 数据中心
    - 开放事件表：开放事件表的字段中放开了虚拟属性。
    - 埋点事件：预定义事件支持关联自定义的事件属性，以支持 SDK 上报页面属性并在分析工具中使用
