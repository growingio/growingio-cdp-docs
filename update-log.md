...
- 新增
  - 企业门户
    - 企业管理
      - 企业项目：支持项目可视化创建、删除
    - 空间管理
      - 空间概览：查看空间内活跃用户情况与空间数据授权
      - 空间角色：同一个用户可在不同空间内拥有不同的功能权限
      - 空间日志：查看空间内的所有用户操作记录
    - 技术支持：增加技术支持角色，协助排查问题与系统运营维护升级
    - OpenAPI：支持通过API的方式进行企业管理
  - 增长分析
    - 页面流分析：事件流分析模型中增加“页面流”模式
    - 首复间隔分析：通过自动计算客户的忠诚用户的复购次数阈值、识别不同阶段用户的黄金营销时期，帮助客户提升忠诚用户体量、促进交易额增长
    - 多主体分析：支持切换事件属性/虚拟属性/用户属性作为新的分析主体，在事件分析模型中进行分析
  - 客户数据平台
    - 标签管理
      - 实时计算：新建标签时设置计算类型为“实时计算”的标签，标签值随实时事件即时更新，满足下游千人千面、销售辅助等应用
    - 群组管理
      - 资产圈群：根据用户资产（即拥有物，如房、车）条件进行人群筛选
    - 客户分析
      - 融合分析：展示用户多身份融合的概览
      - 个体分析：新增“融合历史”合“用户资产”页
  - 数据中心
    - 用户资产：支持创建“人-房-车”的用户资产数据模型，支持资产圈群（也叫以物圈人）
    - 数据源
      - 数据源归因配置：支持网页应用、小程序应用、移动应用配置归因模型及归因窗口
    - UTM管理
      - 自定义维度：可在UTM参数后配置自定义维度，并可在智能分析中解析相关维度
    - 数据集成
      - IP及域名数据过滤：通过过滤可以去除内部人员使用对数据造成的干扰，避免tracking code被拷贝所造成的数据污染。 
 
  - 独立应用
    - 渠道价值分析：新增独立分析应用
      - 基于分析云的全域oneid识别，实现跨渠道用户打通，通过utm参数进行全流量渠道识别与归因
      - 自动生成分析报告
        - 双维度质量评估（转化效果、留存效果）
        - 自动识别最佳ROI渠道、潜力渠道、表现不佳渠道、刷量渠道，为渠道优化提供数据依据
        - 针对具体渠道可自动生成下一维度的质量报告（如关键词、活动名称等），识别具体渠道下哪些关键词、活动表现更佳，作为优化投放策略的参考
        - 适用场景广，适用于线上线下、公域广告投放、私域活动的效果评估
  - 开放平台
     - kafka订阅：支持按项目开启kafka订阅功能，支持按项目设置数据的订阅范围
     - 离线数据导出：支持按项目 T+1导出离线数据，数据类型支持：事件数据/用户属性/用户身份/用户标签/用户分群5类数据
     - OpenAPI接口：客户数据平台增加三个接口：1）用户行为轨迹查询；2）多用户标签值查询；3）用户标签枚举值查询
  - 智能运营
    - 活动策略：
      - 支持「完成A未完成B」活动类型，在特定时间段内，用户完成行为A后一段时间段内未完成行为B，则进入活动流程；
      - 支持基于用户特征（属性和标签）、用户行为、行为序列以及已有群组进行人群细分，当用户满足不同属性值或者行为时流入不同的策略中
    - 触点管理：支持阿里云短信和sendcloud邮件通道，可以通过简单化的界面配置完成触点的接入

- 优化
  - 数据模型
    - 开放元数据表：支持项目级字段区分不同项目的元数据信息
  - 增长分析
    - 各个分析模型交互统一优化，提升体验
    - 漏斗分析：支持次数口径
    - 事件分析：支持聚合时间统计（0～23小时、周一～周日、1～31日）、支持自由下钻优化用户体验
  - 客户数据平台
    - 标签管理
      - 新建标签流程优化，将基本信息和定义规则两个步骤合二为一
    - 群组管理
      - 新建群组流程优化，将基本信息和定义规则两个步骤合二为一
  - 数据中心
    - 虚拟属性：增加字符串转列表、日期转时间戳等函数支持
    - 虚拟属性：支持使用事件属性，用户属性共同创建
    - 维度表字段：增加整数、小数、日期、列表等数据类型
    - 数据源管理：增加京东小程序数据源
    - 用户属性：支持关联维度表
    - 数据脱敏管理：支持区分不同项目角色的用户在应用中是否受脱敏的影响（在项目角色权限中配置）
  - OpenAPI
    - 客户数据平台：优化接口路径和参数，支持多项目、多空间的调用

## v4.0: 2023 年 02 月 28 日发布
...
- 新增
  ...
  - 统一门户
    - 企业管理
      - 企业设置
        - Webhook 通道：支持 Webhook 通道配置，用于身份验证时验证码发送通道。
        - 找回密码：支持多因素身份验证自主找回密码
  - 获客分析
    - 获客追踪
      - 深度链接：深度链接适用于促活、唤回场景，对于 App 已安装，希望通过广告推广促进用户打开 App 或是召回老客户，可通过该方式来创建深度链接（DeepLink）来促使用户回到 App 中。
    - 产品配置
      - 深度链接配置：通过配置预定义行为到唯一的链接上，让用户通过点击链接无缝跳转到特定的内容页面。仅支持 App.
  - 数据中心
    - 数据安全
      - 营销合规限制
        - 支持针对不同空间、不同营销触点创建营销合规规则。营销合规规则启用后，智能运营的营销计划触发范围会收合规规则的影响
        - 支持对营销合规规则进行修改、启用、禁用、管理
      - 个人同意记录
        - 支持导入、存储用户个人同意数据。数据定义包括：同意时间、同意内容、同意状态、同意授权品牌
        - 支持通过根据用户身份信息，查询个人同意记录
...
- 优化
  - 统一门户
    - 企业管理
      - 企业设置
        - 安全设置
          - 支持自定义密码规则，密码复杂度，密码重复次数限制，密码有效期设置
          - 支持二次登录验证
          - 支持防暴力破解，某时间段内密码错误达到上限后锁定账号
          - 支持登录态时长限制
  - 获客分析
    - 获客追踪
      - 监测链接：小程序监测链支持小程序短链
  - 客户数据平台
    - 群体管理
      - 群组画像：支持群组画像 TGI 查看，展示该群组的各特征与全部用户的对应特征的显著程度
      - 群组用户列表：支持人群包对外导出的信息授权管理，包括授权的周期、字段、脱敏等
  - 数据中心
    - 元数据管理
      - 用户身份：用户身份管理支持对用户身份的新增、编辑、优先级拖拽排列等操作

## v2.11: 2022 年 09 月 28 日发布
...
- 新增
  - 用户模型：为满足多渠道营销用户识别的使用需求，对用户模型进行升级
    - 支持唯一身份 ID 和弱身份 ID
    - 支持自定义用户身份和置信度
    - 支持根据多用户身份唯一识别系统用户
  - ​ 用户分群列表：支持固定列设置，支持选择固定首、固定前 2 列、固定前 3 列
  - 用户标签：支持 SQL 标签
  - 项目数据授权：支持按照集成数据源授权和用户属性范围授权
  - ​ 用户删除管理：遵循 GDPR 欧盟规范，支持指定用户删除底层数据
...
- 新增
  - Token 管理：个人中心获取 Token，用于 Chrome 插件登录
...
- 新增
  ...
  - 预警：可以在新事件分析里创建预警
  - 数据
    - ​Mobile Debugger 工具，方便埋点工程师查看移动端 SDK 上传的数据信息
...
- 新增
  - 标签
    - ​ 标签支持分类管理
  - 用戶
    - 腾讯画像，支持第三方腾讯 TGI 人群画像。
    - 用户信息融合到预置用户属性，支持在分析等场景中使用
    - 新增用户身份模块，支持查看 GrowingIO 用户识别配置方案
  - 新版事件分析
    - 埋点事件支持添加单事件过滤
    - 事件分析中支持直接创建计算指标
    - 计算指标新建时支持添加单事件过滤
...
- 新增
  ...
  - 企业管理 - 企业设置
    ...
    - webhook 配置：Webhook 方便企业将通知提醒发送至其它系统，如 IM 工具、短信服务，邮件服务。
- 优化
  - webhook 添加根据状态筛选
  - 帐号申请删除全部选项
  - 修改企业角色排序
...
- 新增：
  ...
  - 多项目管理 ：
    - 支撑大型集团、大型多部门客户建立多个项目，分别进行独立的数据分析与协作。
...
- 新功能:
  ...
  - 全局指标：新增支持总页面停留时长、平均页面停留时长、跳出次数
