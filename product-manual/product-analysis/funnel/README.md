...
假设漏斗步骤是ABCD，如果一个用户的真实路径如下：

ABABCD：用户行为序列中 top of the funnel 为事件第一个事件 A， 找在转化周期内，从 A 到目标事件的所有事件是否按照ABCD的顺序完成了转化。我们会找用户行为序列的第一个A，第一个B，第一个C，第一个D。

如何定义漏斗的转化时长？

首先寻找行为序列的第一个A 到第一个D，然后用第一个D发生的时间减去第一个A发生的时间，结果即为转化时长。
