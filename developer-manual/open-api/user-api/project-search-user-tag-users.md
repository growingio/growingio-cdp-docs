...
## 示例 1：用户标签标识符不存在或未授权该空间

场景：根据空间ID WlGk4Daj 搜索 **未定义** 或 **未授权** 的用户标签 **tag_rfm** 的用户ID，单次查询返回数据上限为100，标识位为0。
...
场景：根据空间ID WlGk4Daj 搜索 群体画像 **tag_rfm** ,单次查询返回数据上限为100，标识位为0。
...
场景：根据空间ID WlGk4Daj 搜索 群体画像 **tag_rfm** 的用户ID、性别、RFM标签,单次查询返回数据上限为100，标识位为100。
